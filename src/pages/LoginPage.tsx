...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      const response = await sheetClient.login(data.username, data.password)

      if (response.status === 'success' && response.data) {
        // ເຊຟລົງ Zustand store (ແລະ localStorage ອັດຕະໂນມັດ)
        login(response.data as User)
        toast.success('ເຂົ້າສູ່ລະບົບສຳເລັດ', {
          description: `ຍິນດີຕ້ອນຮັບ, ${(response.data as User).displayName}`,
        })
        navigate('/', { replace: true })
      } else {
        toast.error('ເຂົ້າສູ່ລະບົບບໍ່ສຳເລັດ', {
          description: response.message || 'ຊື່ຜູ້ໃຊ້ ຫຼື ລະຫັດຜ່ານບໍ່ຖືກຕ້ອງ',
        })
      }
    } catch (error: any) {
  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center px-6">
      ...
      <div className="mb-8 text-center">
        ...
        <h1 className="text-2xl font-bold gradient-text">Jo-Money</h1>
        <p className="mt-1 text-sm text-muted-foreground">ບັນທຶກລາຍຮັບລາຍຈ່າຍ</p>
      </div>
      ...
      >
        ...
        <button
          type="submit"
          disabled={isSubmitting}
          className="w-full flex items-center justify-center rounded-xl bg-primary py-3 text-sm font-semibold text-primary-foreground transition-all hover:bg-primary/90 active:scale-[0.98] shadow-lg shadow-primary/20 disabled:opacity-70 disabled:active:scale-100"
        >
          {isSubmitting ? (
            <>
